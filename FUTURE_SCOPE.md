...
- **Person-reID Market-1501 concept demo** - `person_reid_market1501` workflow with live OpenRouter boss/final-ranker calls + query-side specialists + per-specialist retrievers over offline attribute DBs; `person_reid_market1501_eval` uses precomputed query attributes for a 100-query/500-gallery partition. `evals/person_reid_market1501/` contains `dataset_eval.yaml`, partition and attribute DB builders, gitignored partition/DB/index paths, and a `map_cmc` retrieval scorer (mAP, CMC@1/5/10 with proper junk filtering). Pipeline includes `rrf_precompute` (weighted RRF, attribute weight=10 vs stub weight=1) and `parse_final_ranking` (JSON parser + regex fallback → `ranked_gallery_ids`). Attribute retriever uses Jaccard token-overlap for `color`/`clothing` compound descriptions; exact match for `gender`/`age_group`. Final ranker receives `query_attributes` in prompt. Verified eval baseline: **mAP=34.5%, CMC@1/5/10=35%** (100q/500g partition, placeholder visual/text/body stubs).
